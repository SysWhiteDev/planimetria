...
const Wrapper = ({children, className}: WrapperProps): React.JSX.Element => {
    const [ItemsArray, setItemsArray] = useState([{active: true}, {active: true}, {active:true }]);
        ...
        <div className={`flex items-center ${className}`}>
            ...
            <div className={"flex items-center gap-1.5"}>
                {ItemsList?.map((item, index) => {
                    ...
                    return (
                        <ItemRender index={index} active={ItemsArray[index]?.active} array={ItemsArray}
...
const ItemRender = ({active, setArray, array, index, children, className}: ItemRenderProps): React.JSX.Element => {
    const toggleActive = (index: number) => {
        setArray(array.map((item: { active: any; }, i: number) => {
            if (i === index) {
    ...
    return (
        <div onClick={() => toggleActive(index || 0)}
            ...
            {active && <div className={"bg-white text-black rounded-full text-sm bg-opacity-40 ml-1"}>
                <HiOutlineXMark/>
            </div>}
        </div>
    );
